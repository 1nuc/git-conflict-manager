//! Git Conflict Manager
//! The library is intended to solve the recurring conflict issues in git
//! The crate offers four main strategies to solve the conflict problem which are:
//! 1. Accept local: it will accept the local changes and ignore the foreign branch's changes.
//! 2. Accept Foreign: it will accept the foreign branch's changes and ignore the local changes.
//! 3. combination: it will combine both branch's changes and remove the markers (straightforward).
//!    All of which methodologies can cause a complex, diverged or broken trees in the terminal
//! 4. Trees in git are essentials as they draw the chronological modification made on a Repository
//!    Then the commit is the stamp that provides extra more details such as the user who made the
//!    commit, time of when the commit was made and signature.
//!    Due to that, another advanced method targeted for experienced developers was introduced which
//!    is Tree Merge.
//!    Tree Merge merges the tree for either (head, foreign) branches with the past tree or the
//!    ancestor.
//!    The user get to chose the base tree (head, foreign), he also get to chose whether the
//!    interference of the past (parent) should be enabled or not.
//!    if parent interference is enabled then the current tree selected by the user is directly merged
//!    to the ancestor of the conflicted branches, resulting in a smooth straightforward tree.
//!    The method is super useful for expert developers as they don't need to worry about bucking up
//!    a specific branch if a conflict occurs, they simply chose parent interference and the branch
//!    will be merged with the changes of direct ancestor.
use std::{
    cell::RefMut, fs::remove_file, path::{Path, PathBuf}, time::{SystemTime, UNIX_EPOCH}
};

use git2::{Commit, Error, Index, Oid, Repository, Signature, Status, StatusOptions, Time};

use crate::git_src::Branches;
pub mod combine;
pub mod discarding;
pub mod git_src;
pub mod merge_trees;
pub mod utils;

/// Trait to simply call the methodologies in a simple straightforward manner
pub trait GitOps {
    fn call_discarding(&self, version: bool);
    fn call_combinition(&self);
    fn call_tree_merge(&self, version: bool, parent_interference: bool);
}

/// The initialization trait for which the repository is built
pub trait Initialize {
    fn init(branch_1: String, branch_2: String) -> Self;
    fn return_path() -> PathBuf;
    fn return_repo(file_path: PathBuf) -> Option<Repository>;
    fn does_conflict_exists(&self) -> bool;
}
/// The utils trait is used by the utility module
pub trait Utils {
    fn find_ancesistor(&self, other_branch: &str) -> Result<Oid, Error>;
}

/// the manual control trait that inherits the Actions trait defines the a manual commiting action.
pub trait ManualControl: Actions {
    fn perform_manual_commit(&mut self, version: bool) -> bool {
        let msg = format!(
            "Resolve Conflict: Merge {} branch into {} branch",
            self.branches().src_branch,
            self.branches().dest_branch
        );
        /// get the heads commits
        /// retreive the commits of "ours" branch and theres
        let ours_parents_commits = self
            .repo()
            .head()
            .expect("unable to find the head branch")
            .peel_to_commit()
            .expect("error peeling to commit in ours version")
            .id();
        let theirs_parents_commits = self
            .repo()
            .find_reference("MERGE_HEAD")
            .expect("unable to find the second theirs reference")
            .peel_to_commit()
            .expect("error in peeling to a commit in theirs version")
            .id();
        /// retreive the commits of "theirs" branch

        let parent_commits: &[Oid] = match version {
                true => &[ours_parents_commits],
                false => &[ours_parents_commits,theirs_parents_commits],
            };

        self.commit(parent_commits, msg)
    }
}
/// A trait that contains the necessary action required by all methodologies to resolve a conflict
pub trait Actions {
    //Return the index
    fn branches(&self) -> Branches;

    fn index(&self) -> RefMut<'_, Index>;

    //Return the repo
    fn repo(&self) -> RefMut<'_, Repository>;

    //staging changes
    //this function has an embedding implementation
    fn staging(&mut self, files: Vec<String>) {
        let mut index = self.index();
        let _ = files
            .iter()
            .map(|x| {
                let path = Path::new(x);
                index
                    .add_path(path)
                    .expect("Error adding the file to the staging area");
            })
            .collect::<Vec<_>>();
        index
            .write()
            .expect("unable to save the staged changes to memory");
    }

    ///Making a commit
    ///this function has an embedding implementation
    #[allow(unused_must_use)]
    #[allow(non_snake_case)]
    fn commit(&mut self, parent_commits: &[Oid], msg: String) -> bool {
        let repo = self.repo();
        let tree = repo.find_tree(self.index().write_tree().unwrap()).unwrap();
        // Grabbing the details of the commit
        let signature = repo.signature().unwrap().to_owned();
        let config = repo
            .config()
            .expect("Error in retreiving the configuration")
            .snapshot()
            .expect("error in creating a snapshot");
        // Considering the actual system time for establishing a commit
        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_secs() as i64;

        // Grabbing the author's details of the commit
        let user_name = config.get_str("user.name").unwrap();
        let email = config.get_str("user.email").unwrap();
        let time = Time::new(now, 0);
        let author = Signature::new(user_name, email, &time).expect("unable to create a borrow");
        let p_cm = parent_commits
            .iter()
            .map(|x| repo.find_commit(*x).unwrap())
            .collect::<Vec<Commit>>();
        let p_cm = p_cm.iter().collect::<Vec<&Commit>>();
        let p_commits = p_cm.as_slice();
        //rust git2 doesn't automatically clean up the conflict the conflict must be deleted

        match repo.commit(Some("HEAD"), &author, &signature, &msg, &tree, p_commits) {
            Ok(_val) => {
                //after making the commit git must know that the commit is clearing the conflict
                //therefore, MERGE_HEAD file must be deleted to indicate the success of the merge
                let merge_head_path = repo.path().join("MERGE_HEAD"); // I believe this is the
                // line where the error
                // stems
                //repo.path outputs the content of the .git directory
                //join "MERGE_HEAD" finds the file that starts with MERGE HEAD
                if merge_head_path.exists() {
                    remove_file(merge_head_path).expect("unable to remove the file");
                    //deleting the merge conflict if the commit didn't auto delete
                }
                true
            }
            Error=>{
                println!("{:?}", Error);
                false
            }
        }
    }

    ///checking whether a conflict exists or not
    fn does_conflict_exists(&self) -> bool {
        self.index().has_conflicts()
    }

    ///return the file with a specific conditions, eg., Conflicted
    fn return_conflicted_files(&self, condition: Status) -> Option<Vec<String>> {
        let repo = self.repo();
        let mut options = StatusOptions::new();
        options
            .include_untracked(false)
            .recurse_untracked_dirs(false);
        let status = repo.statuses(Some(&mut options)).unwrap();
        let mut list_of_conflicted_files = Vec::new();
        for i in status.iter() {
            if i.status().contains(condition)
                && let Some(path) = i.path()
            {
                list_of_conflicted_files.push(path.to_owned());
            }
        }
        Some(list_of_conflicted_files)
    }
}
